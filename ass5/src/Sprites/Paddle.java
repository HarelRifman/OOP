...
public class Paddle implements Sprite, Collidable {
    ...
    public void removeBall(Ball b) {
        ...
        int newSize = 0;
        // First, determine the new size of the array
        for (Ball ball : this.balls) {
            if (ball != null && !ball.getCenter().equals(b.getCenter())) {
                newSize++;
            }
        }

        Ball[] newBalls = new Ball[newSize];
        int index = 0;
        // Populate the new array with the balls except the one to be removed
        for (Ball ball : this.balls) {
            if (ball != null && !ball.getCenter().equals(b.getCenter())) {
                newBalls[index++] = ball;
            }
    ...
    /**
     * Sets the array of balls associated with this paddle.
     *
     * @param balls the array of balls to set
     */
        ...
        if (rectangle.getUpperLeft().getX() - Game.BORDER_WIDTH - speed >= 0) {
            ...
            for (Ball b : this.balls) {
                if (b.getCenter().getY() >= rectangle.getUpperLeft().getY()) {
                    if (b.getCenter().getX() <= Game.SCREEN_WIDTH - Game.BORDER_WIDTH
                            && b.getCenter().getX() >= Game.SCREEN_WIDTH
                            - (Game.BORDER_WIDTH + rectangle.getWidth() + speed)) {
                        return;
                    }
                }
            }
            rectangle = new Rectangle(new Point(Game.SCREEN_WIDTH - Game.BORDER_WIDTH - rectangle.getWidth() - 1,
                    rectangle.getUpperLeft().getY()), rectangle.getWidth(), rectangle.getHeight());
        }
    }
        ...
        for (Ball b : this.balls) {
            if (b.getCenter().getY() >= rectangle.getUpperLeft().getY()) {
                if (b.getCenter().getX() >= rectangle.getUpperLeft().getX() + i * speed
                    ...
                    if (i == 1) {
                        b.setVelocity(this.hit(b, new Point(this.rectangle.getUpperLeft().
                                getX() + rectangle.getWidth(),
                                b.getY()), b.getVelocity()));
                        return true;
                    }
                    b.setVelocity(this.hit(b, new Point(this.rectangle.getUpperLeft().getX(), b.getY()),
                            b.getVelocity()));
                    return true;
                }
            }
    ...
    public void moveRight() {
        if (rectangle.getUpperLeft().getX() + rectangle.getWidth() + speed + Game.BORDER_WIDTH <= Game.SCREEN_WIDTH) {
            if (checkBallFromSide(1)) {
                return;
            }
            rectangle = new Rectangle(new Point(rectangle.getUpperLeft().getX() + speed,
